...
async function fetchAllCandidates(db: Databases): Promise<any[]> {
  const docs: any[] = [];
  let offset = 0;
 ...
 * This allows unauthenticated clients (web dashboard) to read candidates.
 */
...
export async function POST(_request: NextRequest) {
  try {
    ...
    console.error("fix-candidates-permissions error:", err);
    return NextResponse.json(
